...
>
> 每天台北早上排程觸發，彙整**「最近一個已收盤的美股交易日」**的數據與新聞（以美東時間計算，自動跳過週末與美股假日）。例如台北週一早上的報告，涵蓋的是上週五的美股盤。

## 功能概覽

| 報告 | 排程（Asia/Taipei） | 涵蓋對象 | 說明 |
|------|------|------|------|
| **美股日報** | 週一至週五 07:30 | 最近一個美股交易日的收盤 | 三大指數 + 七巨頭 + 8 大產業 + 技術指標 + 市場廣度 + GPT-4o 分析 |
| **美股新聞快訊** | 週一至週五 07:35 | 同上 | 指數快照 + Finnhub/Yahoo 新聞 + GPT-4o-mini 重要性篩選 |

> 報告標題會標明資料所屬的美股交易日；footer 會顯示報價覆蓋率（取得幾支 / 預期幾支）。同一交易日被排程重複觸發會自動去重（手動 `/stock`、`/flash` 與 `RUN_NOW` 例外，永遠會重跑）。
...
| `STATE_DIR` | 選用 | 執行狀態與每日快照的存放目錄（預設 `./data`；檔案系統唯讀時自動略過，不影響運作） |
| `RUN_NOW` | 選用 | 設為 `true` 啟動後立即執行報告（會略過「同交易日已產出」的去重） |
...
- **交易日判定**：以美東時間算出「最近一個已收盤的美股交易日」（現在還沒到 16:00 ET 就往前一天，再跳過週末與美股假日）。所有資料抓取視窗、GPT prompt、報告標題都對齊這個交易日，新聞與數據不會錯位。
  - 例：台北週二早上 → 美股週一盤；台北週一早上 → 美股**上週五**盤；台北週二早上但週一是美股假日 → 仍是上週五盤（不會空跑也不會重複）。
- **去重**：排程觸發時若該交易日已產出過就跳過；`/stock`、`/flash` 手動觸發與 `RUN_NOW` 不受此限。
- **每日快照**：每次成功產出會在 `STATE_DIR`（預設 `./data`）寫入：
  - `data/state.json` — 最近已產出的交易日（重啟後仍記得，避免重複推播）
  - `data/reports/<交易日>-stock.json` / `-flash.json` — 當日數據摘要、報價覆蓋率、GPT 內文等
  - 檔案系統唯讀（部分 PaaS）時這些寫入會被安靜略過，不影響推播。注意：Zeabur 等平台**重新部署後容器檔案會清空**，這份快照僅在同一個部署生命週期內持久（要長期保存需另接外部儲存）。
...
**對齊「彙整前一交易日」這個產品目標**
- **交易日錨點**：報告對象固定為「最近一個已收盤的美股交易日」（以美東時間計算，跳週末與美股假日）。修正先前在 UTC 伺服器（如 Zeabur）上**台北週一的報告會被誤判成週末而整個跳過、美股週五盤面從不被彙整**的問題。
- **資料對齊**：Finnhub 市場/個股新聞、Yahoo 個股新聞、Perplexity 宏觀/催化劑搜尋、GPT prompt、報告標題全部改用該交易日為基準（不再是「UTC 昨天～今天」這種會錯位的視窗）。
- **新聞時間真實化**：Finnhub 個股新聞改用回傳的真實 `datetime`（先前塞 `Date.now()`）；Yahoo 新聞依交易日過濾。
- **資料完整度揭露**：日報 footer 顯示報價覆蓋率（取得幾支 / 預期幾支），偏低時加警告；盤中以 `/stock` 手動觸發時報告會註明「以下為前一交易日收盤資料」。
- **去重**：同一交易日被排程重複觸發會自動跳過；`/stock`、`/flash` 與 `RUN_NOW` 不受限。
- **每日快照與狀態**：新增 `STATE_DIR`（預設 `./data`），寫入 `state.json`（最近已產出的交易日）與 `reports/<交易日>-{stock,flash}.json`（當日數據摘要 + GPT 內文）；檔案系統唯讀時自動略過。
- 時間字串改用明確時區（台北/美東），不再依賴伺服器本地時區。
